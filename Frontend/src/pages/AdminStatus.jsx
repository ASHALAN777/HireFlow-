  ...
  return (
    <DashboardLayout title="Status Board">
      ...
      <div className="grid grid-cols-4 gap-4">
        {columns.map((col) => (
          <div key={col} className="bg-white border border-gray-100 rounded-xl p-4">

            <div className="flex items-center justify-between mb-4">
              <p className="text-xs font-medium text-gray-700 capitalize">{col}</p>
              <p className="text-xs text-gray-400">{getByStatus(col).length}</p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              {getByStatus(col).length === 0 ? (
                ...
                <div key={app._id} className="border border-gray-100 rounded-lg p-3">
                  <p className="text-xs font-medium text-gray-900 mb-1">{app.candidate?.name}</p>
                  <p className="text-xs text-gray-400 mb-2">{app.job?.title}</p>
                  <select
                    value={app.status}
