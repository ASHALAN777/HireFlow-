  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5 text-center">
          <p className="text-2xl font-medium text-gray-900">{applications.length}</p>
          <p className="text-xs text-gray-400 mt-1">Total Applicants</p>
        </div>
        <div className="bg-white border border-gray-100 rounded-xl p-5 text-center">
          <p className="text-2xl font-medium text-gray-900">{applications.filter(a => a.status === "interview").length}</p>
          <p className="text-xs text-gray-400 mt-1">Interviews</p>
        </div>
        <div className="bg-white border border-gray-100 rounded-xl p-5 text-center">
          <p className="text-2xl font-medium text-gray-900">{applications.filter(a => a.status === "hired").length}</p>
      ...
      <div className="flex gap-3 mb-8">
        <Link to="/admin/jobs" className="text-xs bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
          Post New Job
        </Link>
        <Link to="/admin/status" className="text-xs border border-gray-200 text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-50">
          View Status Board
        </Link>
      </div>
      ...
      {/* Recent Applications */}
      <p className="text-sm font-medium text-gray-900 mb-4">Recent Applications</p>
