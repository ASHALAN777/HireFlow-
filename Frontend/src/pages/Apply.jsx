...
export default function Apply() {
  ...
  useEffect(() => {
    if (!success) return;
    if (countdown === 0) { navigate(dashboardPath); return; }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!resumeUrl) {
      setError(useProfileResume ? "No resume on profile. Please upload one." : "Please upload your resume first");
      return;
    }
    setLoading(true);
    setError("");
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="min-h-[80vh] flex items-center justify-center">
          <div className="text-center">
            ...
            <button onClick={() => navigate(dashboardPath)} className="text-xs bg-gray-900 text-white px-6 py-2 rounded-lg hover:bg-gray-700">
              Go to dashboard
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        <Link to={`/jobs/${jobId}`} className="text-xs text-gray-400 hover:text-gray-600 mb-6 inline-block">
          ← Back to job
        </Link>

        {job && (
          <div className="mb-8">
            <h1 className="text-2xl font-medium text-gray-900 mb-1">Apply for {job.title}</h1>
            <p className="text-xs text-gray-400">{job.postedBy?.name} · {job.location} · {job.salary}</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {user?.resumeUrl && (
              <div className="flex gap-2 mb-4">
                {[{ label: "Use profile resume", val: true }, { label: "Upload new resume", val: false }].map(({ label, val }) => (
                  <button
                    key={label}
                    type="button"
            ...
            {useProfileResume && user?.resumeUrl ? (
              <div className="border border-gray-200 rounded-lg px-4 py-3 flex items-center justify-between">
                <p className="text-xs text-gray-700">Resume from your profile ✓</p>
                <a href={`${import.meta.env.VITE_API_URL}${user.resumeUrl}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:text-blue-600">
                  Preview
                </a>
              </div>
            ) : (
              <div>
                <div className="flex items-center gap-3">
                  <input
                    type="file"
                    accept=".pdf"
                    onChange={(e) => { setResume(e.target.files[0]); setResumeUrl(""); }}
                ...
                {resumeUrl && !useProfileResume && <p className="text-xs text-green-500 mt-1">✓ Resume uploaded</p>}
            ...
            {!user?.resumeUrl && (
              <p className="text-xs text-gray-400 mt-2">
                💡 Upload your resume on your <Link to="/profile" className="text-blue-500 hover:text-blue-600">profile</Link> to reuse it across applications
              </p>
            )}
          </div>
          ...
          {/* Cover Letter */}
          <div>
            <p className="text-xs text-gray-400 mb-2">Cover letter (optional)</p>
            <textarea
              value={coverLetter}
